        ...
        for layer_info in self.layers_info[1:]:
            if layer_info['type'] == 'linear':
                ...
                states.append({
                    ...
                    'description': f'Computing z = Wx + b for {name}',
                ...
                states.append({
                    ...
                    'description': f'Output activations from {name}',
                    'values': self.activations[name],
                    'layer_idx': layer_idx
                })
...
class NeuralNetworkVisualizer(Gtk.Window):
    ...
    def _on_draw(self, widget, cr):
        ...
        if state['type'] == 'input':
            self._draw_input_layer(cr, width, height, state)
        elif state['type'] in ['forward_pre', 'forward_post']:
            self._draw_forward_propagation(cr, width, height, state)
        elif state['type'] == 'backward_output':
            self._draw_backward_output(cr, width, height, state)
    ...
    def _draw_forward_propagation(self, cr, width, height, state):
        """Draw forward propagation visualization"""
        values = state['values'].numpy() if isinstance(state['values'], torch.Tensor) else state['values']
        if len(values.shape) > 1:
            values = values.flatten()
    ...
    def _draw_backward_output(self, cr, width, height, state):
        """Draw backward propagation output gradients"""
