...
class NetworkAnalyzer:
    ...
    def _analyze_network(self):
        ...
        # Analyze each layer - track Linear and Activation pairs
        prev_was_linear = False
        linear_name = None
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, nn.Linear):
                ...
                prev_was_linear = True
                linear_name = name if name else 'Linear'
            elif prev_was_linear and isinstance(module, (nn.ReLU, nn.Sigmoid, nn.Tanh, nn.LeakyReLU, nn.ELU, nn.GELU)):
                # Found activation following Linear layer
                self.layers_info[-1]['activation'] = module
                self.layers_info[-1]['activation_name'] = type(module).__name__
                prev_was_linear = False
    ...
    def run_forward_backward(self):
        ...
        def forward_hook(module, input, output):
            ...
            if isinstance(module, nn.Linear):
                # Store pre-activation (z = Wx + b)
                self.activations[name + '_pre'] = output.detach().clone()
            elif isinstance(module, (nn.ReLU, nn.Sigmoid, nn.Tanh, nn.LeakyReLU, nn.ELU, nn.GELU)):
                # Store post-activation (a = f(z))
                prev_linear_name = self._get_prev_linear_name(module)
                if prev_linear_name:
                    self.activations[prev_linear_name + '_post'] = output.detach().clone()
    ...
    def _get_prev_linear_name(self, activation_module):
        """Find the Linear layer that precedes this activation"""
        found_activation = False
        prev_linear = None
        
        for name, module in self.model.named_modules():
            if isinstance(module, nn.Linear):
                prev_linear = name if name else 'Linear'
            if module is activation_module:
                found_activation = True
                break
        
        return prev_linear if found_activation else None
    ...
    def create_visualization_states(self) -> List[Dict]:
        ...
        for layer_info in self.layers_info[1:]:
            if layer_info['type'] == 'linear':
                ...
                # Pre-activation state (z = Wx + b)
                pre_activation_key = name + '_pre'
                if pre_activation_key in self.activations:
                    states.append({
                        'step': len(states),
                        'type': 'forward_pre',
                        'title': f'Layer {layer_idx}: Weight Application',
                        'description': f'Computing z = Wx + b for {name}. Weight values shown on connections.',
                        'values': self.activations[pre_activation_key],
                ...
                # Post-activation state (a = f(z))
                post_activation_key = name + '_post'
                has_activation = 'activation' in layer_info
                
                if has_activation and post_activation_key in self.activations:
                    # Has explicit activation function
                    states.append({
                        'step': len(states),
                        'type': 'forward_post',
                        'title': f'Layer {layer_idx}: Activation Output',
                        'description': f'Output after {layer_info["activation_name"]}. Weight values shown on connections.',
                        'values': self.activations[post_activation_key],
                        'pre_activation_values': self.activations[pre_activation_key],
                        'activation_name': layer_info['activation_name'],
                        'layer_idx': layer_idx
                    })
                elif pre_activation_key in self.activations:
                    # No activation function - linear output is final
                    states.append({
                        'step': len(states),
                        'type': 'forward_post',
                        'title': f'Layer {layer_idx}: Linear Output',
                        'description': f'Output from {name} (no activation). Weight values shown on connections.',
                        'values': self.activations[pre_activation_key],
                        'pre_activation_values': self.activations[pre_activation_key],
...
class NeuralNetworkVisualizer(Gtk.Window):
    ...
    def _formula_forward_post(self, state):
        ...
        layer_idx = state['layer_idx']
        values = state['values']
        activation_name = state.get('activation_name', 'Unknown')
        pre_activation_values = state.get('pre_activation_values')
        ...
        if pre_activation_values is not None:
            if isinstance(pre_activation_values, torch.Tensor):
                pre_activation_values = pre_activation_values.numpy()
            if len(pre_activation_values.shape) > 1:
                pre_activation_values = pre_activation_values.flatten()
        ...
        if activation_name == 'Identity':
            formula += "No activation function applied (linear layer output):\n\n"
            formula += "SYMBOLIC FORMULA:\n"
            formula += "  a = z  (identity)\n\n"
        else:
            formula += f"Applying {activation_name} activation function:\n\n"
            formula += "SYMBOLIC FORMULA:\n"
            formula += "  a = f(z)\n\n"
            formula += "Where:\n"
            formula += "  z = pre-activation values (from Wx + b)\n"
            formula += f"  f = {activation_name} activation function\n"
            formula += "  a = activated outputs\n\n"
            
            # Explain the activation function
            if activation_name == 'ReLU':
                formula += "ReLU(z) = max(0, z)\n"
                formula += "  - Outputs z if z > 0\n"
                formula += "  - Outputs 0 if z ≤ 0\n\n"
            elif activation_name == 'Sigmoid':
                formula += "Sigmoid(z) = 1 / (1 + e^(-z))\n"
                formula += "  - Outputs values in range (0, 1)\n\n"
            elif activation_name == 'Tanh':
                formula += "Tanh(z) = (e^z - e^(-z)) / (e^z + e^(-z))\n"
                formula += "  - Outputs values in range (-1, 1)\n\n"
            elif activation_name == 'LeakyReLU':
                formula += "LeakyReLU(z) = max(αz, z)\n"
                formula += "  - Outputs z if z > 0\n"
                formula += "  - Outputs αz if z ≤ 0 (small slope)\n\n"
        
        # Numerical comparison
        formula += "NUMERICAL VALUES:\n\n"
        
        if pre_activation_values is not None and activation_name != 'Identity':
            formula += "Transformation from z to a:\n"
            for i in range(min(len(values), 8)):
                z_val = pre_activation_values[i]
                a_val = values[i]
                
                if activation_name == 'ReLU':
                    if z_val <= 0:
                        formula += f"  z{i} = {z_val:7.3f}  →  a{i} = {a_val:7.3f}  (zeroed)\n"
                    else:
                        formula += f"  z{i} = {z_val:7.3f}  →  a{i} = {a_val:7.3f}  (passed)\n"
                else:
                    formula += f"  z{i} = {z_val:7.3f}  →  a{i} = {a_val:7.3f}\n"
            ...
            formula += "\n"
            if activation_name == 'ReLU':
                num_zeroed = sum(1 for z in pre_activation_values if z <= 0)
                num_active = len(pre_activation_values) - num_zeroed
                formula += f"Summary: {num_active} neurons active, {num_zeroed} zeroed\n"
        else:
            formula += "Post-activation values:\n"
            for i in range(min(len(values), 8)):
    ...
    def _draw_full_network_forward(self, cr, width, height, state):
        ...
        # Add all Linear layers with post-activation values when available
        for idx, layer_info in enumerate(self.analyzer.layers_info[1:], 1):
            if layer_info['type'] == 'linear':
                name = layer_info['name']
                # Prefer post-activation values, fall back to pre-activation
                post_key = name + '_post'
                pre_key = name + '_pre'
                
                if post_key in self.analyzer.activations:
                    values = self.analyzer.activations[post_key].numpy()
                elif pre_key in self.analyzer.activations:
                    values = self.analyzer.activations[pre_key].numpy()
                else:
                    # Fallback for compatibility
                    values = self.analyzer.activations.get(name, torch.zeros(layer_info['size'])).numpy()
